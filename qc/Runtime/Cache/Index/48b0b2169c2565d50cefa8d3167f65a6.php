...
<!--左侧 end-->
    ...
    <tbody>
		<?php if(!empty($noover)): if(is_array($noover)): $i = 0; $__LIST__ = $noover;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><tr>
			...
			<td><?php if(($v["buystatus"]) == "0"): ?><form action="__PLAY__/alipayapi.php" method="post">
					<input type="hidden" name="name" value="<?php echo ($v["name"]); ?>" />
					<input type="hidden" name="booknum" value="<?php echo ($v["booknum"]); ?>" />
					<input type="hidden" name="number" value="<?php echo ($v["number"]); ?>" />
					<input type="hidden" name="allprice" value="<?php echo ($v["allprice"]); ?>" />
					<input type="submit" value='' alt="立即支付" name="sub" class="zhifubao" />
				
				</form>
			
			<?php else: ?><img src="__PUBLIC__/images/images/ok.png"style="width:10px;"/><?php endif; ?></td>
			...
			<td><?php if(($v["bookstatus"]) == "0"): ?><a href="<?php echo U(GROUP_NAME.'/IndexUser/saveBook',array('id'=>$v['id']));?>" class="ajax-get"><img src="__PUBLIC__/images/images/save.png"style="width:15px;"/></a><?php endif; ?>
				
				<a href="<?php echo U(GROUP_NAME.'/IndexUser/delBook',array('id'=>$v['id']));?>" class="confirm ajax-get"><img src="__PUBLIC__/images/images/del.png"style="width:15px;"/></a>
                </td>
    ...
    <tbody>
		<?php if(!empty($over)): if(is_array($over)): $i = 0; $__LIST__ = $over;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><tr>
			...
			<td><?php echo (date('y-m-d H:i',$v["addtime"])); ?></td>
			<td>
				<?php if(($v["bookstatus"]) == "0"): ?><a href="<?php echo U(GROUP_NAME.'/IndexUser/saveBook',array('id'=>$v['id']));?>" class="ajax-get"><img src="__PUBLIC__/images/images/save.png"style="width:15px;"/></a><?php endif; ?>
				<a href="<?php echo U(GROUP_NAME.'/IndexUser/delBook',array('id'=>$v['id']));?>" class="confirm ajax-get"><img src="__PUBLIC__/images/images/del.png"style="width:15px;"/></a>
                </td>
		</tr><?php endforeach; endif; else: echo "" ;endif; ?>
