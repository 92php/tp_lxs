<?php if (!defined('THINK_PATH')) exit();?><title><?php echo ($list['title']); ?></title>
...
<div class="hotel">
	...
	<div class="hotel_content">
		<div class="foshi_cont_top"><h6><a href=""><?php echo ($list["fenlei"]); ?></a></h6></div>
		<div class="cont_main">
			<h1><a href=""><?php echo ($list["title"]); ?></a></h1>
			
				
				<div class="foshi_yudimg"><a style="display:block;width:300px;margin:50px auto;text-align:center;font-size:18px;color:#5e5e5e;text-decoration:none; margin-top:-10px; margin-left:-80px;" href="javascript:void(0);" data-reveal-id="myModal"><img src="__PUBLIC__/images/images/me_yud.jpg" /></a></div>
			<!--弹出框-->
			<div id="myModal" class="reveal-modal">
				<h2>预定表单</h2>
				<form action="<?php echo U(GROUP_NAME.'/Category/cateYud');?>" method="post">
				<p>预定名称：<input name="title" type="text" class="input" size="20" value="<?php echo ($list["fenlei"]); ?>：<?php echo ($list["title"]); ?>"></p>
				...
				<p>会议时间：<input class="Wdate" type="text" onClick="WdatePicker()" name="startime" style="width:180px;height:30px;"> </p>
				<p>参会人数：<input name="shu" type="text" class="input" size="35" value=""></p>
				<p>　　备注：<textarea class="input" name="note" style="width:180px; height:70px;"></textarea></p>
				<input type="hidden" name='type' value='会议订单'/>
			...
			<div class="con">
				...
				<div class="page"><p style="width:340px; margin-left:0px;">上一篇：<a href="<?php echo U(GROUP_NAME.'/Category/cateShow', array('id' => $prev['id']));?>"><?php if($prev): echo ($prev["title"]); else: ?>没有了<?php endif; ?></a></p><p style="width:340px; margin-right:0px;
				margin-top:-37px;">下一篇：<a href="<?php echo U(GROUP_NAME.'/Category/cateShow', array('id' => $next['id']));?>"><?php if($next): echo ($next["title"]); else: ?>没有了<?php endif; ?></a></p></div>
	</div>
	
	<div class="huiyi_callme">
