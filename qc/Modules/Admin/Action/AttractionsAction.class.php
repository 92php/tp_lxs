...
class AttractionsAction extends CommonAction{
	...
	/* 添加景点表单接受处理 */
	Public function runAttractions(){
		if(!$this->isPost()){
			halt('页面不存在');
		}
		if($_POST['title']!=''&&$_POST['about']!=''&&$_POST['content']!=''){
			...
			$data=array(
				...
				'about'=>$this->_post('about','htmlspecialchars'),
	...
	Public function saveAttractions(){
		...
		$w=array('id'=>$id);
		$db=M('attractions');
		$jingdian=$db->where($w)->select();
		/* p($hotel);die; */
		$this->assign('jingdian',$jingdian);
		$this->display();
	}
	/* 景点编辑接受表单 */
	Public function sendAttractions(){
		if(!$this->isPost()){
			halt('页面不存在');
		}
		if(empty($_POST)){
	...
	/* 景点删除 */
	Public function delAttractions(){
		$w=array('id'=>$_GET['id']);
		if(M('attractions')->where($w)->limit('1')->delete()){
	...
	/* 景点检索 */
	Public function sechAttractions(){
		if(isset($_GET['sech'])){
			$where=array('title'=>array('LIKE','%'.$this->_get('sech').'%'));
			$attr=M('attractions')->where($where)->select();
			//p($attr);die;
			$this->attr=$attr?$attr:false;
		}
		$this->display();
	}
